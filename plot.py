...
class FactorStrategyPlotter:
    def __init__(self,
        ...
        title = f'{data["metric1"].metric} | {data["metric2"].metric} | ' \
                    f'{data["metric1"].operator} | {details["model"]} | ' \
                    f'{data["metric1"].asset_input}->{data["metric1"].asset_output} | ' \
                    f'{details["currency"]} | {details["resolution"]} | {details["direction"]} | ' \
                    f'{details["long_short"]} | ' \
                    f'Num trades of best sharpe: {round(target_df.iloc[0]["trade"])} | {details["since"]} to {details["until"]}'

        self.fig.update_layout(
            title_text=title,  # Main title
            title_x=0.5,  # Center the title
            title_font_size=11.5,  # Font size
