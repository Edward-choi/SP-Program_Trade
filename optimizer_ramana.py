...
class Optimizer:
    ...
    def _model_window(self, model: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        model_params = {
            "bband": {
                "24h": ((5, 100, 5), (0, 2.6, 0.1)),
                "1h": ((5, 2400, 100), (0, 2.25, 0.1)),
            ...
            "ma_diff": {
                "24h": ((5, 100, 5), (0, 0.11, 0.01)),
                "1h": ((5, 2400, 100), (0, 0.1, 0.01)),
                "10m": ((300, 12000, 300), (0, 0.1, 0.01)),
            },
            "roc": {
                "24h": ((5, 100, 5), (0, 0.21, 0.01)),
                "1h": ((5, 2400, 100), (0, 0.1, 0.005)),
                "10m": ((1, 14, 1), (0, 0.01, 0.001)),
            },
            "rsi": {
                "24h": ((5, 100, 5), (0, 102.5, 2.5)),
                "1h": ((5, 2400, 100), (0, 102.5, 2.5)),
                "10m": ((0, 20, 1), (0, 30, 1)),
            },
            "percentile": {
                "24h": ((5, 100, 5), (0, 0.525, 0.025)),
                "1h": ((5, 2400, 100), (0, 0.55, 0.03)),
            ...
            "min_max": {
                "24h": ((5, 100, 5), (0.00, 0.125, 0.025)),
                "1h": ((5, 2400, 100), (0, 0.5, 0.03)),
                "10m": ((10, 110, 10), (0.05, 0.125, 0.05)),
            },
            "raw": {
                "24h": ((5, 100, 5), (0.00, 0.525, 0.025)),
                "1h": ((5, 2400, 100), (0.05, 0.5, 0.05)),
                "10m": ((10, 110, 10), (0.05, 0.5, 0.05)),
            },
            "2ma_cross": {
                "24h": ((10, 375, 10), (10, 375, 10)),
                "1h": ((10, 10000, 300), (10, 375, 10)),
                "10m": ((1, 14, 1), (0, 0.01, 0.001)),
            },
        }
...
class FactorStrategyOptimizer(Optimizer):
    def __init__(self, **kwargs):
        ...
        self.resolution = self.strategy_info.resolution
        self.long_short = self.strategy_info.long_short
        self.currency = self.strategy_info.currency
        self.data = kwargs.get("data_preprocessed", pd.DataFrame())
        self.metric_info: Dict[str, MetricDetail] = kwargs.get("metric_info")
        self.asset = self.metric_info.get("metric1").asset_input
        ...
        for window in window_list:
            try:
                for threshold in threshold_list:
                    ...
                    temp_df = cal_pnl(temp_df, self.resolution)
    ...
    def __show_top_strategy(self, model):
        ...
        metric = self.metric_info["metric1"].metric
        folder = f'results/{metric}/{model}/{self.asset}/{self.direction}/{self.long_short}/{self.resolution}'
        os.makedirs(folder, exist_ok=True)
        since = time.strftime('%Y-%m-%d', time.gmtime(self.since))
        until = time.strftime('%Y-%m-%d', time.gmtime(self.until))
        csv_file = f'results_{since}_{until}.csv'
        result_df.to_csv(os.path.join(folder, csv_file))
        temp_df = copy.deepcopy(self.data)
        while True:
            print(
                f"\n******************** Top 150 Sharpe Equity Curve with model:    {model}  | {self.strategy_info.direction}  ***************************")
            ...
            print(
                f"******************** Top 150 Sharpe Equity Curve with model:    {model} | {self.strategy_info.direction}  ***************************")

            scan = input("\nEnter 1 to 150, 'c' to continue, 'q' or 'x' to exit: ").strip()
            ...
            if isinstance(scan, int) and 1 <= scan <= 150:
                ...
                fig = plot.FactorStrategyPlotter(
                    df_plot=df_placeholder, mode="advanced", result=result, data=self.metric_info, details={"since": since, "until": until, "model": model,
                        "asset": self.asset, "direction": self.direction, "long_short": self.long_short, "resolution": self.resolution, "currency": self.currency}, target_df=target_df
                )
                fig.show()
        ...
        try:
            ...
            title = f'{self.metric_info["metric1"].metric} | {self.metric_info["metric2"].metric} | ' \
                    ...
                    f'{self.strategy_info.long_short}\n  \
                                              Num trades of best sharpe: {round(target_df.iloc[0]["trade"])} | {since} to {until}' 
            plt.title(title)
            metric = self.metric_info["metric1"].metric
            folder = f'results/{metric}/{model}/{self.asset}/{self.direction}/{self.long_short}/{self.resolution}'
            os.makedirs(folder, exist_ok=True)
            image_file = f'results_{since}_{until}.jpg'
            plt.savefig(os.path.join(folder, image_file), format='jpg', dpi=100)
            plt.close()
