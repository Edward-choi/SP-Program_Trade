...
from optimizer_ramana import FactorStrategyOptimizer
...
class FactorStrategy(Strategy):
    ...
    def optimize(self, since: int, until: int) -> None:
        ...
        # if not self.data:
        if True:
            # one_year_ago = datetime.fromtimestamp(since)- timedelta(days=365)
            data: Dict[str, pd.DataFrame] = self.fetch_raw_data(
                # since=int(one_year_ago.timestamp()),  # Start time
